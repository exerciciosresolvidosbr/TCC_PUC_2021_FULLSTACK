...
import br.com.condagil.repository.query.ReceitaQueryRepository;

@Service
public class ReceitaQueryService {

	@Autowired
	private ReceitaQueryRepository receitaQueryRepository;
	
	public List<ReceitaDTO> recuperarReceitas(){
		List<ReceitaDTO> listaReceitasDto = ReceitaDTOBuilder
				.converterListaEntityEmReceitaDTO(receitaQueryRepository.recuperarReceitas());
		return listaReceitasDto;
	}

	public ReceitaDTO recuperarReceitaPorId(Long receitaId) {
		return ReceitaDTOBuilder.getInstance()
				.converterEntityEmReceitaDTO(receitaQueryRepository.recuperarReceitaPorId(receitaId))
				.getReceitaDTO();
	}
	
	public ByteArrayInputStream exportarReceitasPdf() {
		List<ReceitaDTO> listaReceitasDto = recuperarReceitas();
		...
		try {
			...
			Paragraph paragraph = new Paragraph("Lista de Receitas" , font);
			...
			for(ReceitaDTO dto: listaReceitasDto) {
				...
				PdfPCell dataCell = new PdfPCell(new Phrase(dto.getDataReceita()));
